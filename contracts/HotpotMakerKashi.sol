...
// HotpotMakerKashi is MasterChef's left hand and kinda a wizard. He can cook up Hotpot from pretty much anything!
// This contract handles "serving up" rewards for xHotpot holders by trading tokens collected from Kashi fees for Hotpot.
contract HotpotMakerKashi is Ownable {
    ...
    function _convertStep(address token0, uint256 amount0) private returns (uint256 hotpotOut) {
        if (token0 == hotpot) {
            IERC20(token0).safeTransfer(bar, amount0);
            hotpotOut = amount0;
        } else if (token0 == weth) {
            hotpotOut = _swap(token0, hotpot, amount0, bar);
        } else {
            ...
            hotpotOut = _convertStep(bridge, amountOut);
        }
    }
