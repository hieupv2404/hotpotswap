...
contract CloneRewarderTime is IRewarder,  BoringOwnable{
    ...
    function onHotpotReward (uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) onlyMCV2 lock override external {
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 hotpotReward = time.mul(rewardPerSecond);
            accToken1PerShare = accToken1PerShare.add(hotpotReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 hotpotReward = time.mul(rewardPerSecond);
                pool.accToken1PerShare = pool.accToken1PerShare.add((hotpotReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
