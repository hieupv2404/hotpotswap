...
task("migrate", "Migrates liquidity from Uniswap to HotpotSwap")
...
task("bar:enter", "HotpotBar enter")
.addParam("amount", "Amount")
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  ...
  await run("erc20:approve", { token: hotpot.address, spender: bar.address })
  
  await (await bar.connect(await getNamedSigner("dev")).enter(amount)).wait()
});

task("bar:leave", "HotpotBar leave")
.addParam("amount", "Amount")
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  const hotpot = await ethers.getContract("HotpotToken")
...
task("maker:serve", "HotpotBar serve")
.addParam("a", "Token A")
.addParam("b", "Token B")
