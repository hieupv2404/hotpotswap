...
describe("KashiHotpotMaker", function () {
  before(async function () {
    await prepare(this, ["HotpotMakerKashi", "HotpotBar", "HotpotMakerKashiExploitMock", "ERC20Mock", "UniswapV2Factory", "UniswapV2Pair", "BentoBoxV1", "KashiPairMediumRiskV1", "PeggedOracleV1"])
  })
  ...
  beforeEach(async function () {
    ...
    // Deploy Hotpot and Kashi contracts
    ...
    await deploy(this, [["kashiMaker", this.HotpotMakerKashi, [this.factory.address, this.bar.address, this.bento.address, this.hotpot.address, this.weth.address, this.factory.pairCodeHash()]]])
    await deploy(this, [["exploiter", this.HotpotMakerKashiExploitMock, [this.kashiMaker.address]]])
    ...
    await this.hotpot.approve(this.bento.address, getBigNumber(10))
    ...
    await this.bento.deposit(this.hotpot.address, this.alice.address, this.alice.address, getBigNumber(10), 0)
    ...
    //const initData = defaultAbiCoder.encode(["address", "address", "address", "bytes"], [this.hotpot.address, this.dai.address, this.oracle.address, oracleData])
    //await this.bento.deploy(this.KashiMaster.address, initData, true)
  })

  describe("setBridge", function () {
    it("only allows the owner to set bridge", async function () {
      await expect(this.kashiMaker.connect(this.bob).setBridge(this.hotpot.address, this.weth.address, { from: this.bob.address })).to.be.revertedWith("Ownable: caller is not the owner")
    })
    
    it("does not allow to set bridge for Hotpot", async function () {
      await expect(this.kashiMaker.setBridge(this.hotpot.address, this.weth.address)).to.be.revertedWith("Maker: Invalid bridge")
    })

    it("does not allow to set bridge for WETH", async function () {
      await expect(this.kashiMaker.setBridge(this.weth.address, this.hotpot.address)).to.be.revertedWith("Maker: Invalid bridge")
    })
    ...
    it("emits correct event on bridge", async function () {
      await expect(this.kashiMaker.setBridge(this.dai.address, this.hotpot.address))
  ...
  describe("convert", function () {
    it("reverts if caller is not EOA", async function () {
      await expect(this.exploiter.convert(this.hotpot.address)).to.be.revertedWith("Maker: Must use EOA")
    })
  })
})
