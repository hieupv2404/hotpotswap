...
describe("MasterChefV2", function () {
  before(async function () {
    await prepare(this, ["MasterChef", "HotpotToken", "ERC20Mock", "MasterChefV2", "RewarderMock", "RewarderBrokenMock"])
    await deploy(this, [["brokenRewarder", this.RewarderBrokenMock]])
  })

  beforeEach(async function () {
    await deploy(this, [["hotpot", this.HotpotToken]])

    await deploy(this, [
      ["lp", this.ERC20Mock, ["LP Token", "LPT", getBigNumber(10)]],
      ["dummy", this.ERC20Mock, ["Dummy", "DummyT", getBigNumber(10)]],
      ["chef", this.MasterChef, [this.hotpot.address, this.bob.address, getBigNumber(100), "0", "0"]],
    ])
    ...
    await deploy(this, [
      ["chef2", this.MasterChefV2, [this.chef.address, this.hotpot.address, 1]],
  ...
  describe("PendingHotpot", function () {
    it("PendingHotpot should equal ExpectedHotpot", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      ...
      await advanceBlock()
      let expectedHotpot = getBigNumber(100)
        .mul(log2.blockNumber + 1 - log.blockNumber)
        .div(2)
      let pendingHotpot = await this.chef2.pendingHotpot(0, this.alice.address)
      expect(pendingHotpot).to.be.equal(expectedHotpot)
    })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef2.poolInfo(0)).accHotpotPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of HOTPOT and reward", async function () {
      ...
      let expectedHotpot = getBigNumber(100)
        .mul(log2.blockNumber - log.blockNumber)
        .div(2)
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedHotpot)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.hotpot.balanceOf(this.alice.address))
        .to.be.equal(await this.r.balanceOf(this.alice.address))
        .to.be.equal(expectedHotpot)
    })
    ...
    it("Harvest for HOTPOT-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedHotpot)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.hotpot.balanceOf(this.alice.address)).to.be.equal(expectedHotpot)
    })
  })
