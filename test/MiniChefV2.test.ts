...
describe("MiniChefV2", function () {
  before(async function () {
    await prepare(this, ["MiniChefV2", "HotpotToken", "ERC20Mock", "RewarderMock", "RewarderBrokenMock"])
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ...
      ["chef", this.MiniChefV2, [this.hotpot.address]],
    ...
    await this.hotpot.mint(this.chef.address, getBigNumber(10000))
    await this.lp.approve(this.chef.address, getBigNumber(10))
    await this.chef.setHotpotPerSecond("10000000000000000")
  ...
  describe("PendingHotpot", function () {
    it("PendingHotpot should equal ExpectedHotpot", async function () {
      ...
      let expectedHotpot = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      let pendingHotpot = await this.chef.pendingHotpot(0, this.alice.address)
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef.poolInfo(0)).accHotpotPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of HOTPOT and reward", async function () {
      ...
      let timestamp = (await ethers.provider.getBlock(log.blockNumber)).timestamp
      let expectedHotpot = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      expect((await this.chef.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedHotpot)
