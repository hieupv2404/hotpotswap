...
describe("Timelock", function () {
  ...
  it("should not allow non-owner to do operation", async function () {
    await this.hotpot.transferOwnership(this.timelock.address)
    // await expectRevert(this.hotpot.transferOwnership(carol, { from: alice }), "Ownable: caller is not the owner")

    await expect(this.hotpot.transferOwnership(this.carol.address)).to.be.revertedWith("Ownable: caller is not the owner")
    await expect(this.hotpot.connect(this.bob).transferOwnership(this.carol.address)).to.be.revertedWith("Ownable: caller is not the owner")
  ...
  it("should do the timelock thing", async function () {
    ...
    await this.timelock
      .connect(this.bob)
      .queueTransaction(this.hotpot.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    await increase(duration.days(1))
    await expect(
      this.timelock
        .connect(this.bob)
        .executeTransaction(this.hotpot.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    ).to.be.revertedWith("Timelock::executeTransaction: Transaction hasn't surpassed time lock.")
    await increase(duration.days(4))
    await this.timelock
      .connect(this.bob)
      .executeTransaction(this.hotpot.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    expect(await this.hotpot.owner()).to.equal(this.carol.address)
  })
